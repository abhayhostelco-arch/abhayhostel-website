...
The browser is untrusted. Roles, user IDs, date ranges, sort choices, and account state are resolved or allowlisted on the server. Supabase RLS is the final authorization boundary, and server actions repeat role and ownership checks. The service-role key is server-only and reserved for Auth administration, profile state changes, and immutable audit writes.
