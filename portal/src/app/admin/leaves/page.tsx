...
import { getLeaveNotificationDeliveries, getLeaveRequests, getPrivateUploadSignedUrl, getProfiles } from "@/lib/data";
...
export default async function LeaveManagementPage({ searchParams }: { searchParams: Promise<{ month?: string; studentId?: string; status?: string }> }) {
  ...
  const notifications = actor.role === "super_admin" ? await getLeaveNotificationDeliveries(result.requests.map((request) => request.id)) : [];
  ...
  return <main className="page-container">
    ...
    {!result.available ? <section className="panel"><div className="empty-state unavailable-state"><strong>Leave Management Unavailable</strong><p>Apply the supplied database migration to enable this feature.</p></div></section> : <>
      ...
      <section className="panel section-gap"><div className="panel-title"><h2>Leave Applications</h2><span>{result.requests.length} requests</span></div><div className="table-wrap"><table><thead><tr><th>Student</th><th>Dates</th><th>Leave Days</th><th>Reason</th><th>Application</th><th>Status / Decision</th></tr></thead><tbody>{result.requests.map((request) => { const student = students.find((item) => item.id === request.student_id); const notification = notifications.find((item) => item.leave_request_id === request.id && item.decision_version === request.decision_version); return <tr key={request.id}><td>{student ? <Link className="table-primary-link" href={studentProfilePath(actor.role === "super_admin" ? "super_admin" : "admin", student.id)}>{student.full_name}</Link> : "Student"}</td><td>{displayDate(request.start_date)} – {displayDate(request.end_date)}</td><td>{leaveDays(request)}</td><td>{request.reason}</td><td>{urls.get(request.id) ? <a href={urls.get(request.id)!} target="_blank" rel="noreferrer">View</a> : "—"}</td><td><LeaveDecisionControls requestId={request.id} studentName={student?.full_name ?? "student"} status={request.status} decisionNote={request.decision_note} canDecide={actor.role === "super_admin"} notificationId={notification?.id} notificationStatus={notification?.status} canRetry={actor.role === "super_admin"} /></td></tr>; })}</tbody></table>{!result.requests.length ? <div className="empty-state"><strong>No Leave Requests</strong><p>No applications match these filters.</p></div> : null}</div></section>
    </>}
  </main>;
}
