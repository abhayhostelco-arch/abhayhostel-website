...
import { evidencePurgedMessage } from "@/lib/daily-entry-upload";
...
export default async function StudentDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ range?: string; date?: string }> }) {
  ...
  const hasStudentGroup = isStudentGroup(student.student_group);
  const activeGroupStudents = hasStudentGroup ? activeStudents.filter((profile) => profile.student_group === student.student_group) : [];
  const start = daysAgoInIndia(range - 1);
  const activeScoringEntries = await getEntries({ startDate: start, studentIds: activeGroupStudents.map((profile) => profile.id), completeScoringWeeks: true });
  const studentScoringEntries = student.is_active && hasStudentGroup ? activeScoringEntries.filter((entry) => entry.student_id === id) : await getEntries({ startDate: start, studentId: id, completeScoringWeeks: true });
  const entries = studentScoringEntries.filter((entry) => entry.entry_date >= start);
  ...
  const personalGrowth = buildGrowthReport([{ ...student, is_active: true }], studentScoringEntries, scoreSettings, range).students[0];
  const activeGrowth = hasStudentGroup ? buildGrowthReport(activeGroupStudents, activeScoringEntries, scoreSettings, range).students.find((report) => report.studentId === id) : undefined;
  ...
  const evidenceUrls = new Map(await Promise.all(entries.filter((entry) => entry.maha_mantra_path).map(async (entry) => [entry.id, await getPrivateUploadSignedUrl("maha-mantra-evidence", entry.maha_mantra_path)] as const)));
  const correctionEvidenceUrl = correctionEntry ? evidenceUrls.get(correctionEntry.id) ?? null : null;
  ...
  return (
    <main className="page-container">
      ...
      <section className={`student-overview-grid section-gap-small${personalGrowth ? "" : " student-overview-grid-single"}`}>
        ...
        {personalGrowth ? <section className="panel student-growth-summary"><div className="panel-title"><h2>Growth Score</h2><span>{hasStudentGroup && activeGrowth?.rank ? `Rank #${activeGrowth.rank} of ${activeGroupStudents.length} · ${studentGroupLabel(student.student_group)}` : hasStudentGroup ? "Historical report" : "Group migration required"}</span></div><GrowthScoreCards scores={personalGrowth} /></section> : null}
      </section>
      ...
      <section className="panel section-gap"><div className="panel-title"><h2>Daily records</h2></div><div className="table-wrap"><table><thead><tr><th>Date</th><th>Bedtime</th><th>Wake-up</th><th>Sleep</th><th>Study</th><th>Rounds</th><th>Morning Arati</th><th>Gita class</th><th>Reading</th><th>Class attended</th><th>Seva</th><th>Note</th></tr></thead><tbody>{entries.map((entry) => <tr key={entry.id}><td>{displayDate(entry.entry_date)}</td><td>{entry.sleep_time.slice(0, 5)}</td><td>{entry.wake_time.slice(0, 5)}</td><td>{formatMinutes(sleepDurationMinutes(entry.sleep_time, entry.wake_time))}</td><td>{formatMinutes(entry.study_minutes)}</td><td>{entry.chanting_rounds ?? "—"}</td><td>{entry.morning_arati_status ?? (entry.morning_arati_attended ? "present" : "absent")}{evidenceUrls.get(entry.id) ? <> · <a className="table-primary-link" href={evidenceUrls.get(entry.id) ?? undefined} target="_blank" rel="noreferrer">View image</a></> : entry.maha_mantra_purged_at ? ` · ${evidencePurgedMessage}` : ""}</td><td>{entry.gita_class_status.replace("_", " ")}</td><td>{entry.evening_reading_minutes}m</td><td>{entry.library_attended ? "Yes" : "No"}</td><td>{entry.seva_minutes}m</td><td>{entry.note ?? "—"}</td></tr>)}</tbody></table>{entries.length === 0 ? <p className="empty-state">No entries in this range.</p> : null}</div></section>
