...
export default async function StudentDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ range?: string; date?: string }> }) {
  ...
  const activeScoringEntries = await getEntries({ startDate: start, studentIds: activeStudents.map((profile) => profile.id), completeScoringWeeks: true });
  ...
  const activeGrowth = buildGrowthReport(activeStudents, activeScoringEntries, scoreSettings, range).students.find((report) => report.studentId === id);
