...
export default async function StudentDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ range?: string; date?: string }> }) {
  ...
  const activeGroupStudents = activeStudents.filter((profile) => profile.student_group === student.student_group);
  ...
  const studentScoringEntries = student.is_active ? activeScoringEntries.filter((entry) => entry.student_id === id) : await getEntries({ startDate: start, studentId: id, completeScoringWeeks: true });
  ...
  const activeGrowth = buildGrowthReport(activeGroupStudents, activeScoringEntries, scoreSettings, range).students.find((report) => report.studentId === id);
  ...
  return (
    <main className="page-container">
      ...
      <section className={`student-overview-grid section-gap-small${personalGrowth ? "" : " student-overview-grid-single"}`}>
        ...
        {personalGrowth ? <section className="panel student-growth-summary"><div className="panel-title"><h2>Growth Score</h2><span>{activeGrowth ? `Rank #${activeGrowth.rank} of ${activeGroupStudents.length} · ${studentGroupLabel(student.student_group)}` : "Historical report"}</span></div><GrowthScoreCards scores={personalGrowth} /></section> : null}
      </section>
