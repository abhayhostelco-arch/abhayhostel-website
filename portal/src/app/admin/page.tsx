...
export default async function AdminDashboard({ searchParams }: { searchParams: Promise<{ mentorId?: string; range?: string; startDate?: string; endDate?: string }> }) {
  ...
  return <main className="page-container"><AutoCleanupTrigger />
    ...
    <section className="dashboard-reference-grid section-gap">
      <DashboardPanel title="Overall Progress" description={`Rolling ${range}-day group average`} className="score-panel dashboard-half-panel"><ScoreOverview scores={report.averages} /></DashboardPanel>
      <DashboardPanel title="Gita Attendance" description="Recent official attendance" className="dashboard-half-panel"><AttendanceHeatmap days={heatmapDays} /></DashboardPanel>
