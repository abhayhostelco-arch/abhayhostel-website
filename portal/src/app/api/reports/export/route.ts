...
export async function GET(request: Request) {
  ...
  const students = activeStudents.filter((student) => !parsed.data.studentId || student.id === parsed.data.studentId);
  const entries = await getEntries({ startDate: daysAgoInIndia(range - 1), studentIds: students.map((student) => student.id) });
