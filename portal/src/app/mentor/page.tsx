...
export default async function MentorDashboard() {
  ...
  return <main className="page-container"><header className="page-heading hero-heading"><div><p className="eyebrow">Mentor Workspace</p><h1>Welcome, {mentor.full_name.split(" ")[0]}</h1><p>Review assigned Students and respond to missed entries early.</p></div><div className="heading-actions"><Link className="button button-secondary" href="/mentor/reports?range=30">30-Day Reports</Link><Link className="button" href="/mentor/students">View My Students</Link></div></header><section className="metric-grid"><article className="metric-card"><span><Users size={15} aria-hidden="true" /> Assigned Students</span><strong>{active.length}</strong></article><article className="metric-card"><span><CheckCircle2 size={15} aria-hidden="true" /> Submitted Today</span><strong>{todayEntries.length}</strong></article><article className="metric-card"><span><Clock3 size={15} aria-hidden="true" /> Missing Today</span><strong>{Math.max(active.length - todayEntries.length, 0)}</strong></article><article className="metric-card"><span><BellRing size={15} aria-hidden="true" /> 30-Day Alerts</span><strong>{allAlerts.length}</strong></article></section><section className="dashboard-grid section-gap"><article className="panel"><div className="panel-title"><div><h2>Assigned-Group Completion · Last 30 Days</h2><span>Daily submission percentage for assigned students.</span></div></div><TrendChart data={chartData} mode="completion" /></article><article className="panel"><div className="panel-title"><h2>Students Needing Attention</h2><Link href="/mentor/alerts?range=30">All Alerts</Link></div><div className="alert-list">{alerts.map((alert) => <div className="alert-item" key={alert.id}><BellRing size={18} aria-hidden="true" /><div><strong>{alert.studentName}</strong><span>{alert.date} · {alert.message}</span></div></div>)}{!alerts.length ? <div className="empty-state"><strong>No Recent Alerts</strong><p>Assigned Students are currently on track.</p></div> : null}</div></article></section><section className="panel section-gap"><div className="panel-title"><div><p className="eyebrow">Hostel Scoreboard</p><h2>Top 10 · Rolling 30 Days</h2></div></div><GrowthLeaderboard students={globalReport.students.slice(0, 10)} /></section></main>;
}
