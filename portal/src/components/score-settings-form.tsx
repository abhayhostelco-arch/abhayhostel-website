...
export function ScoreSettingsForm({ settings }: { settings: ScoreSettings }) {
  ...
  return (
    <form action={action} className="split-form">
      {fields.map(([name, label, value, min, max]) => <div className="field" key={name}><label htmlFor={name}>{label}</label><input id={name} name={name} type="number" min={min} max={max} defaultValue={value} required />{state.fieldErrors?.[name]?.map((message) => <p className="field-error" key={message} role="alert">{message}</p>)}</div>)}
      ...
      <p className="field-hint full-span">Category weights must total 100. Changes recalculate reports from the launch date.</p>
